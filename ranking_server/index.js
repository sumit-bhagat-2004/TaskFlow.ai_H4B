...
const ProfessionalSchema = new mongoose.Schema({
  ...
  ratings: { type: RatingSchema, default: () => ({ expertise: [], clarity: [], communication: [], mentorship: [] }) },
  rating_breakdown: {
    expertise: { type: [Number], default: [] },
    clarity: { type: [Number], default: [] },
    communication: { type: [Number], default: [] },
...
const getRatingCount = (pro) => Math.min(
...
app.post("/professionals/:id/rate", async (req, res) => {
  try {
    const { expertise, clarity, communication, mentorship, project_id, project_name } = req.body;

    if ([expertise, clarity, communication, mentorship].some(v => typeof v !== 'number')) {
      return res.status(400).json({ message: "All rating values must be numbers" });
    }

    if (typeof project_id !== 'string' || typeof project_name !== 'string') {
      return res.status(400).json({ message: "Project ID and name must be strings" });
    }
    ...
    const isDuplicateProject = pro.project_details.some(
      p => p.project_id.trim().toLowerCase() === project_id.trim().toLowerCase() ||
           p.project_name.trim().toLowerCase() === project_name.trim().toLowerCase()
    );

    if (isDuplicateProject) {
      return res.status(400).json({ message: "Duplicate project_id or project_name entry not allowed" });
    }
    ...
    const avgExpertise = parseFloat(avg(pro.rating_breakdown.expertise).toFixed(2));
    const avgClarity = parseFloat(avg(pro.rating_breakdown.clarity).toFixed(2));
    const avgCommunication = parseFloat(avg(pro.rating_breakdown.communication).toFixed(2));
    const avgMentorship = parseFloat(avg(pro.rating_breakdown.mentorship).toFixed(2));
    ...
    // Individual rating R
    const R = (avgExpertise + avgClarity + avgCommunication + avgMentorship) / 4;
...
app.get("/ranking", async (req, res) => {
  try {
    const rankedProfessionals = await Professional.find({}, 'name overall_score')
...
app.listen(PORT1, () => console.log(`API running on http://localhost:${PORT1}`));
